  ...
  return (
    <div>
      ...
      <section className="bg-gradient-to-b from-amber-50 to-white px-4 py-16 text-center">
        ...
        <form action="/en/search" method="get" className="max-w-md mx-auto relative">
      ...
      <section className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {categories.map(cat => {
            ...
            return (
              <Link key={cat} href={`/en/category/${categorySlugs[cat]}`} className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md transition">
      ...
      <section className="max-w-6xl mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/en/categories" className="text-sm text-amber-600 hover:underline flex items-center gap-1">View all <ArrowRight size={14} /></Link>
        </div>
        <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-12 gap-2">
          {popular.map(e => (
            <Link key={e.codepoint} href={`/en/emoji/${e.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}`} className="group flex flex-col items-center rounded-lg p-2 hover:bg-amber-50 transition">
